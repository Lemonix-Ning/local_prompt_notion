...
async function testImportToCoding() {
  console.log('========================================');
  console.log('测试导入到 Coding 分类');
  console.log('========================================\n');
  ...
  const targetCategory = path.join(vaultRoot, 'Coding');
  
  console.log('🚀 开始导入...');
  console.log(`   目标分类: Coding`);
  console.log(`   完整路径: ${targetCategory}`);
  ...
  try {
    const response = await fetch(`${API_BASE}/prompts/import`, {
      ...
      body: JSON.stringify({
        prompts,
        categoryPath: targetCategory, // 指定目标分类
    ...
    if (result.success) {
      ...
      console.log('\n📁 预期的目录结构:');
      console.log('   vault/Coding/');
      console.log('   ├── 编程/');
      console.log('   │   ├── Python/');
      console.log('   │   │   ├── python_基础教程/');
      console.log('   │   │   └── python_高级特性/');
      console.log('   │   └── JavaScript/');
      console.log('   │       ├── javascript_es6/');
      console.log('   │       └── React/');
      console.log('   │           └── react_hooks_指南/');
      console.log('   └── 工作/');
      console.log('       ├── 产品管理/');
      console.log('       │   ├── 产品需求文档模板/');
      console.log('       │   └── 用户故事写作/');
      console.log('       └── 团队协作/');
      console.log('           ├── 每日站会模板/');
      console.log('           └── 代码审查清单/');

      console.log('\n💡 提示: 请在应用中查看 Coding 分类下的子目录结构');
    } else {
